...
The `solesense-v2/` foot diagram shows the 3-zone x medial/lateral sensor layout (2 circles per zone) drawn on a cut-out CAD render of the actual insole. Right foot is the same image; both have the live FSR fill so you can verify wiring. Auth screen lives at the front of the SPA -- sign in or create an account before reaching the recording flow.
...
|---|---|---|
| Home / Recording / Report / Settings screens | [x] | [x] |
| Auth (login + self-signup) | -- | [x] |
| `POST /api/start` / `/api/stop` | [x] | [x] |
| Live FSR readout during recording (foot diagram) | [x] | [x] |
| Pause-on-disconnect "Paused" badge | -- | [x] |
| "Disconnected" status indicator | -- | [x] |
| Recording timer source | JS `setInterval` | `/api/run-state.elapsed_ms` (MCU truth) |
| Report from `/data.csv` parse + JS analysis | [x] | -- |
| Report from `/api/run-report` (MCU computes) | -- | [x] |
| AI Coach (LLM-personalised feedback) | -- | [x] |
| `POST /api/calibrate/zero` & `/api/calibrate/imu` UI | [x] | [x] |
| `POST /api/data/clear` UI | [x] | [x] |
| Pressure distribution by zone | [x] (4 zones) | [x] (3 zones x med/lat) |
| Two-foot diagram with anatomical sensor positions | [x] | [x] |
...
- `solesense-v2/`: three zones -- Heel, Midfoot, Forefoot -- matching the v0.2 3-zone x medial/lateral sensor layout (2 sensors per zone).
...
The injury-flag thresholds (cadence < 160 spm, pronation > 15 deg, supination < -8 deg, asymmetry > 10%, impact rate > 80 BW/s) come from peer-reviewed biomechanics research (sources in [`../../SOLESENSE.md`](../../SOLESENSE.md) 13) and aren't user-tunable. The firmware has a `/api/settings` endpoint and NVS-backed thresholds struct for future tooling, but no UI is wired to it.
