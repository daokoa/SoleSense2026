...
- **MPU-6050** 6-axis IMU on I2C
  ...
  - Accel range: +/-2g -> 16384 LSB/g -> converted to m/s^2
  - Gyro range: +/-250 deg/s -> 131 LSB/( deg/s) -> converted to  deg/s

### Power
- **2x LIR2450** 3.7V 120mAh rechargeable Li-ion coin cells, **wired in parallel** per insole
- ~240mAh total capacity
- Active draw (ESP32-C3 + WiFi AP): ~80-160mA -> 90-180 min active
- Deep sleep draw: ~5uA -> weeks of standby
...
CSV text
  ...
  -> cadence (steps/duration x 60)
  -> ground contact time (contact frames / total frames x duration / steps)
...
| Low Cadence | Medium | < 160 spm | Steps / duration x 60 |
| Overpronation | High | > 15 deg | Gyro X integration per step |
| Supination | Medium | < -8 deg | Gyro X integration per step |
...
**Why:**
- 2x LIR2450 in parallel = ~240mAh
- Active + WiFi: 80-160mA -> 90-180 min if always on
- Deep sleep: ~5uA -> weeks of standby
...
- **Souza (2016)** *Journal of Orthopaedic & Sports Physical Therapy* -- overpronation > 15 deg, supination < -8 deg
